...
#define PULSES_PER_REVOLUTION 16
...
uint8_t broadcast_mac[] = {0x5C, 0x01, 0x3B, 0x33, 0x9C, 0x84}; // man hinh me
//uint8_t broadcast_mac[] = {0x14,0x33,0x5C,0x0A,0x9B,0x20}; // man anh nam truong
...
void setup() {
    ...
    // setup led matrix
    display.begin();
    display.setIntensity(5); // Đặt độ sáng tối đa
...
void loop() {
    ...
    if (current_time - last_espnow_send >= SEND_INTERVAL) {
        ...
        char bufferr[5];
        sniprintf(bufferr, sizeof(bufferr), "%d", current_rpm);
        display.print(bufferr);
