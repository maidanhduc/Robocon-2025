...
bool stt_write = true;
uint32_t time_tx = 0;
uint8_t but_data_2 = 0x00;
uint32_t time_but = 0;
uint16_t joy_LX;uint16_t joy_LY;uint16_t joy_RX;uint16_t joy_RY;
uint8_t arr[4][2];
uint8_t num_motor = 4;
bool stt_motor = false;

void UART_get_button(){
  if(Serial3.available()>=3){
    if(Serial3.read()==0xAA){
      data_rx[0] = Serial3.read();
      data_rx[1] = Serial3.read();
      but_data = data_rx[0] | (data_rx[1] << 8);
    }
  }
}
void UART_get_angle(){
...
void uart_get_stm32(){
  if(Serial3.available()){
    Serial3.readBytes((char*)&but_data, 2);
    Serial3.readBytes((char*)&joy_LX, 2);
    Serial3.readBytes((char*)&joy_LY, 2);
    Serial3.readBytes((char*)&joy_RX, 2);
    Serial3.readBytes((char*)&joy_RY, 2); 
  }
}
// mix code by me
...
void ctrl_mode() {
  ...
  if (but_data == 8192) {
    if (flag_select) {
      flag_auto_pwm = true;
      tx_data.mode_ball += 1;
      if (tx_data.mode_ball >= 5) tx_data.mode_ball = 2;
  ...
  if (millis() -  time_delay >= 50){
    ...
    if (flag_auto_pwm){
      if (tx_data.mode_ball >= 1 && tx_data.mode_ball <= 4 &&
        ...
        speed = DEFAULT_PWM[tx_data.mode_ball - 1][tx_data.mode_area - 1];
      }
    }
  }
}
