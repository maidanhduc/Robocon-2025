...
// Biến toàn cục
extern uint16_t but_data = 0x00;
uint32_t bt_in = 0;
int16_t cur_angle;
uint8_t rx_data[2];
uint8_t rx_data_angle[2];
...
void uart_get_button() {
    ...
    if (Serial2.available() >= 2) {
        rx_data[0] = Serial2.read();
        rx_data[1] = Serial2.read();
        but_data = rx_data[0] << 8 | rx_data[1];
    }
}

// Hàm nhận góc
uint16_t uart_get_angle() {
    Serial3.write('z');
    delay(5);
    if(Serial3.available() > 0) {
        rx_data_angle[0] = Serial3.read();
        rx_data_angle[1] = Serial3.read();
        cur_angle = rx_data_angle[0] << 8 | rx_data_angle[1];
...
// Hàm điều khiển mode và cập nhật giá trị
//========================== cài đặt các mode bắn========================//
